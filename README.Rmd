...
- `x`:	This is the list file (read by read_sf from package `sf`), which is originally a shape file with .shp extension. team_11 function will convert it to a dataframe 
- `tolerance`: This controls how much the shape file is thinned. The default is 0.1.
...
# plot 
# library(ggplot2)
# ggplot(tmp, aes(x=long,y=lat,group=group))+geom_polygen()
```
